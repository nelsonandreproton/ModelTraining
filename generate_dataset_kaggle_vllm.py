# generate_dataset_kaggle_vllm.py
# Generates PT-PT Q&A pairs about Portugal using LOCAL Qwen2.5-7B inference via vLLM.
# Optimized for speed on Kaggle T4 GPU(s): continuous batching + prefix caching + AWQ 4-bit.
# Runs entirely on Kaggle — no HF Inference API, no external API calls.
#
# WHY THIS IS FASTER THAN generate_dataset_kaggle.py:
#   1. vLLM continuous batching   → the GPU serves dozens of prompts in parallel instead of
#                                    one prompt at a time (the single biggest win).
#   2. Prefix caching              → the long shared system persona is encoded ONCE and reused
#                                    across every prompt, not re-processed each call.
#   3. AWQ 4-bit quantization      → unlike bitsandbytes NF4 (optimized for training memory),
#                                    AWQ is built for *inference*; on a T4 it lets the 7B model
#                                    fit with room for a large KV-cache, without the per-step
#                                    dequant penalty that slows the old script down.
#   4. 7B instead of 3B            → better factual accuracy and PT-PT adherence → fewer
#                                    rejected pairs → less wasted generation. vLLM recovers the
#                                    speed the larger model would otherwise cost.
#
# ── QUICK START ──────────────────────────────────────────────────────────────
#
# STEP 1 — TEST MODE (always run this first):
#   Set TEST_MODE = True below. It generates ~50 pairs in a couple of minutes and saves to
#   /kaggle/working/generated_pairs.json. If you see progress and a final "Done. N pairs saved"
#   → the full run is safe.
#
# STEP 2 — FULL RUN (only after test passes):
#   Set TEST_MODE = False, TARGET = 2000 (or more).
#   Run the cell. Progress is printed every wave.
#   When done (or when Kaggle session limit approaches), click "Save Version".
#
# STEP 3 — RESUME after a session ends:
#   After "Save Version", add the output dataset as an INPUT dataset in the next session,
#   set RESUME_INPUT_PATH to the mounted path, and run again to continue.
#
# Output: /kaggle/working/generated_pairs.json (incremental saves after every wave)
# ─────────────────────────────────────────────────────────────────────────────

# ── Step 1: Install dependencies ─────────────────────────────────────────────
# vLLM pulls a compatible torch/transformers stack. We install once, write a marker,
...
# ── Config ────────────────────────────────────────────────────────────────────
# Set TEST_MODE = True for a quick sanity check before the full run.
TEST_MODE   = True          # ← CHANGE TO False FOR FULL RUN

TARGET      = 50 if TEST_MODE else 2000
PAIRS_PER_PROMPT = 5        # how many Q&A pairs each prompt asks the model to produce
WAVE_SIZE   = 16 if TEST_MODE else 96   # prompts submitted to vLLM at once (continuous batching)
FRESH_START = False         # ← True = always generate from scratch, ignoring any existing output file

# 7B AWQ: best speed/quality balance on a T4. vLLM serves it with continuous batching.
# Alternatives if AWQ kernels misbehave on your GPU:
#   "Qwen/Qwen2.5-7B-Instruct-GPTQ-Int4"  (set QUANTIZATION="gptq")
#   "Qwen/Qwen2.5-7B-Instruct"            (fp16; set QUANTIZATION=None, needs ~14GB → tight on one T4)
GENERATOR_MODEL  = "Qwen/Qwen2.5-7B-Instruct-AWQ"
QUANTIZATION     = "awq"

OUTPUT_FILE      = "/kaggle/working/generated_pairs.json"
MAX_NEW_TOKENS   = 1024      # enough for 5 short PT-PT pairs; smaller than the old 1500 → faster
TEMPERATURE      = 0.85
TOP_P            = 0.9
...
if torch.cuda.is_available():
    for i in range(torch.cuda.device_count()):
        ...
        print(f"GPU {i}: {props.name}  VRAM: {props.total_memory / 1e9:.1f} GB", flush=True)
print(f"Tensor-parallel size: {NUM_GPUS}", flush=True)
...
# NOTE: the persona is identical across every prompt, so vLLM prefix caching encodes
# it once and reuses it for the whole run — a large part of the speed-up.
_PERSONA = (
...
def parse_valid_pairs(raw: str, existing: set) -> list[dict]:
    """Parse one model output → list of validated {instruction, response} dicts.

    Mutates `existing` so duplicates are rejected across the whole run.
    """
...
print(f"\nLoading {GENERATOR_MODEL} with vLLM "
      f"(quantization={QUANTIZATION}, tp={NUM_GPUS})...", flush=True)
llm = LLM(
    model=GENERATOR_MODEL,
    quantization=QUANTIZATION,          # AWQ: inference-optimized 4-bit (unlike bnb NF4)
    dtype="float16",
...
def build_prompt(category: str, subtopic: str, fmt: dict, n: int, existing: set) -> str:
    """Render one chat prompt (persona + format instruction) into a model-ready string."""
    sample = random.sample(list(existing), min(5, len(existing))) if existing else []
    existing_sample = "; ".join(f'"{s[:50]}"' for s in sample) or "nenhuma"
    user_prompt = fmt["user"].format(
        topic=category, subtopic=subtopic, n=n, existing_sample=existing_sample
    )
    messages = [
...
# ── Main generation loop — wave-based continuous batching ─────────────────────
...
print(f"Topics: {len(TOPIC_SEEDS)} categories, {len(combos)} subtopics", flush=True)
print(f"Wave size: {WAVE_SIZE} prompts × {PAIRS_PER_PROMPT} pairs each\n", flush=True)

wave = 0
while len(results) < TARGET:
    wave += 1
    # Build a whole wave of prompts up front, then let vLLM batch them all at once.
    prompts = []
    for _ in range(WAVE_SIZE):
        category, subtopic = combos[combo_idx % len(combos)]
        combo_idx += 1
        fmt = random.choice(FORMAT_POOL)
        prompts.append(build_prompt(category, subtopic, fmt, PAIRS_PER_PROMPT,
                                    EXISTING_INSTRUCTIONS))

    # Single call → vLLM serves the whole wave with continuous batching.
    outputs = llm.generate(prompts, sampling_params)

    wave_added = 0
    for out in outputs:
        raw = out.outputs[0].text
        pairs = parse_valid_pairs(raw, EXISTING_INSTRUCTIONS)  # dedups via EXISTING_INSTRUCTIONS
        if pairs:
            results.extend(pairs)
            wave_added += len(pairs)
            if len(results) >= TARGET:
                break  # stop mid-wave once the target is reached
    ...
    print(f"[wave {wave:>3}] +{wave_added:>3} valid | total: {len(results):>5}/{TARGET} "
          f"| {fmt_elapsed(elapsed)} | {rate:.0f} pairs/min", flush=True)
